      ...
      {elementsToRender.map((element, index) => {
        switch (element) {
          case CalculatorElementsName.Display: {
            return <WrapperCalculatorComponentInConstructor key={element} order={0} id={element} children={<Display/>}/>;
          }
          case CalculatorElementsName.Equal: {
            return <WrapperCalculatorComponentInConstructor key={element} order={index} id={element} children={<EqualButton/>}/>;
          }
          case CalculatorElementsName.Operators: {
            return <WrapperCalculatorComponentInConstructor key={element} order={index} id={element} children={<OperatorButtons/>}/>
          }
          case CalculatorElementsName.Numbers: {
            return <WrapperCalculatorComponentInConstructor key={element} order={index} id={element} children={<NumberButtons/>}/>
          }
        }
      })}
    </div>
